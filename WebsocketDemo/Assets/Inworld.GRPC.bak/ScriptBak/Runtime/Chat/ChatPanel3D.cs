...
namespace Inworld.Sample.UI
{
    ...
    public class ChatPanel3D : MonoBehaviour
    {
        ...
        void _RefreshBubbles(List<HistoryItem> historyItems)
        {
            foreach (HistoryItem item in historyItems)
            {
                if (!m_Bubbles.ContainsKey(item.UtteranceId))
                {
                    ...
                    {
                        GameObject go = Instantiate(m_LeftBubble, m_PanelAnchor);
                        m_Bubbles[item.UtteranceId] = go.GetComponent<ChatBubble>();
                    ...
                    {
                        GameObject go = Instantiate(m_RightBubble, m_PanelAnchor);
                        m_Bubbles[item.UtteranceId] = go.GetComponent<ChatBubble>();
        ...
        [SerializeField] GameObject m_LeftBubble;
        [SerializeField] GameObject m_RightBubble;
