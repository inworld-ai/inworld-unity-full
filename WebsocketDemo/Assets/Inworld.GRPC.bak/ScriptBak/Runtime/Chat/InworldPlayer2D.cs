...
namespace Inworld.Sample
{
    public class InworldPlayer2D : MonoBehaviour
    {
        ...
        [SerializeField] GameObject m_BubbleLeft;
        [SerializeField] GameObject m_BubbleRight;
        ...
        void _RefreshBubbles(List<HistoryItem> historyItems)
        {
            foreach (HistoryItem item in historyItems)
            {
                if (!m_Bubbles.ContainsKey(item.UtteranceId))
                {
                    ...
                    {
                        GameObject go = Instantiate(m_BubbleLeft, m_ContentRT);
                    ...
                    {
                        GameObject go = Instantiate(m_BubbleRight, m_ContentRT);
                        m_Bubbles[item.UtteranceId] = go.GetComponent<ChatBubble>();
