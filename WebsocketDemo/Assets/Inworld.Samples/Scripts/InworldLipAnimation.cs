...
namespace Inworld.Assets
{
    public class InworldLipAnimation : MonoBehaviour
    {
        ...
        public InworldAudioInteraction m_AudioInteraction;
        ...
        void OnEnable()
        {
            if (!Character || !m_AudioInteraction)
                return;
            
            Character.Interaction.OnInteractionChanged += OnInteractionChanged;
            //Character.onBeginSpeaking.AddListener(OnAudioStarted);
            Character.onEndSpeaking.AddListener(OnAudioFinished);
        }
        void OnDisable()
        {
            if (!Character || !m_AudioInteraction)
            ...
            Character.onEndSpeaking.RemoveListener(OnAudioFinished);
        }
        ...
        public void Init()
        {
            ...
            m_AudioInteraction = GetComponent<InworldAudioInteraction>();
            m_VisemeMap.Clear();
            _MappingBlendShape();
        }
        void _Reset()
        ...
        public void OnInteractionChanged(List<InworldPacket> packets)
        {
            foreach (InworldPacket packet in packets)
            {
                if (packet is AudioPacket ap)
                {
                    OnAudioStarted(ap);
                    break;
                }
        ...
        void OnAudioStarted(AudioPacket audioPacket)
        {
            _Reset();
            // if (!Character || !m_AudioInteraction || m_AudioInteraction.NextAudio == null)
            // {
            //     return;
            // }
            foreach (PhonemeInfo phoneme in audioPacket.dataChunk.additionalPhonemeInfo)
            {
                ...
                m_VisemeMap.Enqueue(new Vector2(visemeIndex, (float)phoneme.startOffset));
            }
        }
